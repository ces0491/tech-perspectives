...
|---|---|---|
| Sonnet 5, medium | $0.04 | 468 tokens |
| Opus 5, high | $0.10 | 825 tokens |
| Opus 5, xhigh | $0.12 | 822 tokens |
...
That context is carried on every later call until you clear. Anthropic's pricing page lists an Opus 5 cache read at $0.50 per million tokens, so 825 tokens comes to $0.0004 a call.

Each session routed the same two short task descriptions. Sonnet 5 at medium recommended the same settings for both as the Opus sessions did, at less than half their cost. Its answers ran to 173 and 175 words against 308 to 371 for Opus.
...
The rename lands in the middle band. It cost $0.25 on Opus at xhigh, moving it to Sonnet at low saved $0.17, and asking cost $0.12, so asking came out $0.05 ahead. That is a gain too small to be worth the typing. The feature, the bug fix and the review each landed where a route saves more than twice its cost.
...
- For a feature, a debugging session or a review, ask at the start of the phase, then `/clear` before you switch, so the switch does not re-process the conversation on the new setting. Switch with `s` in the `/model` picker: typing `/model <name>` also saves it as your default for new sessions.
...
The route costs come from one run per setting. The 1.1.1 route, run twice on identical text, cost $0.155 and $0.129, so two runs of one route can differ by $0.025, against the rename's $0.05 margin. Sonnet 5 at high, the default on Pro and Team Standard plans, was not measured.

*Task runs on Claude Code 2.1.263, 8 September 2026. Routes on tokenwise 1.3.0 and Claude Code 2.1.272. The code, the transcripts and the chart's generator are at [github.com/ces0491/tokenwise](https://github.com/ces0491/tokenwise), and two commands check the figures without spending anything: `node bench/breakeven.mjs --check` confirms the chart follows from the committed runs, and `node bench/skill-cost.mjs --compare 1.3.0,1.3.0-opus-high@1.3.0,1.3.0-sonnet-medium@1.3.0` prints the route costs from the saved sessions.*
